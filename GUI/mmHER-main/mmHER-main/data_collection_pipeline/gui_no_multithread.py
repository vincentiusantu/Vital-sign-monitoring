...
class VitalSignGUI(QWidget):
    ...
    def run_capture_script(self):
        try:
            ...
            self.clear_plot()
            subprocess.run([sys.executable, "data_capture.py", str(duration)], check=True)
            QMessageBox.information(self, "Capture Selesai", f"Data berhasil direkam selama {duration} detik.")
            process_data()
            self.hr_slider.setMaximum(len(slide_hr[0]) - 1)
            self.update_plot()
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Gagal menjalankan capture: {str(e)}")
    ...
    def update_plot(self):
        ...
        for i in range(len(hr_data) - 1):
            self.hr_scene.addLine(i, 50 - hr_data[i]*50, i+1, 50 - hr_data[i+1]*50, QPen(Qt.black))

        self.rr_scene.clear()
        rr_data = slide_rr[self.selected_antenna][self.current_frame]
        for i in range(len(rr_data) - 1):
            self.rr_scene.addLine(i, 50 - rr_data[i]*50, i+1, 50 - rr_data[i+1]*50, QPen(Qt.darkBlue))
